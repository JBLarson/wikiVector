...
class SemanticQualityAnalyzer:
    ...
    def query(self, text: str) -> List[Dict]:
        ...
        try:
            # --- THIS IS THE FIX (1/2) ---
            # Request 'semantic' ranking AND a larger 'k' value (50)
            url = f"{self.base_url}/api/related/{text}?ranking=semantic&k=50"
            # ----------------------------
    ...
    def test_domain_clustering(self, domain: str, articles: List[str]) -> Dict:
        ...
        connections_as_lists = {k: list(v) for k, v in connections.items()}
        # ----------------------------
        ...
        return {
            ...
            'connections': connections_as_lists  # Use the JSON-safe version
        }
                ...
                for r in results:
                    ...
                    if target.lower() in title_lower:
                        ...
                        return {
                            ...
                            'start': start, 'target': target # Add for logging
                        }
                    
                    next_level.add(title)
            
            # Now with k=50, this should be much larger
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Wikipedia Embeddings Semantic Quality Analyzer")
    parser.add_argument(
        '--run_id', 
        type=int, 
        required=True, 
        help="The run_id from the database to log results against"
    )
    ...
    # --- Updated: Log results to database ---
    print("\nLogging results to database...")
    db_logger.log_quality_results(args.run_id, results)
    print(f"✓ Results logged to database for run_id {args.run_id}")
